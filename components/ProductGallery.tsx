...
export default function ProductPage() {
  ...
  // activeImageUrl n'est plus utilisé pour forcer l'affichage, on se base sur userSelectedGalleryImage ou selectedVariation
  ...
  async function loadProduct() {
    try {
      ...
      if (productData.is_variable_product) {
        ...
        if (variations) {
          // Normalisation des attributs et variations (code identique à l'existant)
  ...
  const handleVariationChange = (variation: any) => {
    ...
    setUserSelectedGalleryImage(null); // On déverrouille le mode manuel
    // Note: On ne force plus activeImageUrl ici, on laisse getCurrentImageUrl gérer
  };
  ...
  const handleImageClick = (image: { id: string; src: string }) => {
    ...
    // 2. Identification par URL (Fallback)
    ...
    if (isVariation) {
      ...
      setUserSelectedGalleryImage(null); // Déverrouillage manuel
      ...
      setUserSelectedGalleryImage(image.src); // Verrouillage manuel
    }
  };
  ...
  // LOGIQUE DE SÉLECTION D'IMAGE (PRIORITÉS)
  const getCurrentImageUrl = (): string | undefined => {
    if (!product) return undefined;

    // 1. Si l'utilisateur a cliqué manuellement sur une image "non-variation" (Verrou)
    if (userSelectedGalleryImage) {
      return userSelectedGalleryImage;
    }

    // 2. Si une variation est sélectionnée, on affiche son image
    if (selectedVariation?.image?.src) {
      return selectedVariation.image.src;
    }

    // 3. Fallback : Image principale du produit
  ...
  // CONSTRUCTION DE LA GALERIE (STABLE - NE BOUGE PAS)
  const galleryImages = useMemo(() => {
    if (!product) return [{ id: "placeholder", src: "/placeholder.png", alt: "Product" }];

    const images: Array<{ id: string; src: string; alt: string }> = [];

    // On n'ajoute PAS la variation sélectionnée au début pour éviter que ça "saute"
    // On liste tout simplement tout ce qu'on a, dans un ordre fixe.

    // 1. TOUTES LES VARIATIONS
    ...
    // 2. IMAGE PRINCIPALE (Si pas déjà là)
  ...
  }, [product]); // Note: Dépendance uniquement sur 'product', pas 'selectedVariation', pour la stabilité
