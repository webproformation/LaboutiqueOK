...
// Couleurs de secours pour l'affichage (si la base de données n'a pas de code hex)
...
export function ProductFilters({ categorySlug, activeFilters, onFiltersChange }: ProductFiltersProps) {
  ...
  const loadFilterOptions = async () => {
    try {
      ...
      // CIBLAGE PRÉCIS : On cherche "Principale" pour les couleurs
      ...
      // On charge tous les termes pour éviter de faire 3 requêtes
      ...
      if (allTerms) {
        // COULEURS : Uniquement celles de "Couleur Principale"
        if (mainColorAttr) {
          const colors = allTerms.filter(t => t.attribute_id === mainColorAttr.id);
          ...
          // Fallback : Si pas d'attribut "Principale", on prend tout ce qui a un code couleur
          const fallbackColors = allTerms.filter(t => t.color_code !== null);
           const uniqueColors = Array.from(new Map(fallbackColors.map(item => [item.name, item])).values());
