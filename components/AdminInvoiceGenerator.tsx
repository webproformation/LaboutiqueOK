...
export function AdminInvoiceGenerator() {
  ...
  // --- LOGIQUE DE NUMÉROTATION SÉQUENTIELLE ---
  const getNextSequence = async () => {
    // On cherche la toute dernière facture générée, toutes années confondues
    const { data } = await supabase
      .from('invoices')
      .select('invoice_number')
      .order('id', { ascending: false }) // On suppose que ID auto-incrémente, sinon created_at
      .limit(1)
      .maybeSingle();
    ...
    if (!data || !data.invoice_number) {
      // Aucune facture ? On commence à 1
      return { year: currentYear, sequence: 1 };
    }

    // Format attendu : FA2026-0000001
    ...
    if (parts.length === 2 && parts[0].startsWith('FA')) {
      ...
      if (lastYear === currentYear) {
        return { year: currentYear, sequence: lastSeq + 1 };
      }
    }
    
    // Si nouvelle année ou format inconnu, on reset
    return { year: currentYear, sequence: 1 };
  };
  ...
  const handleGenerateInvoices = async () => {
    ...
    try {
      // On trie les commandes par date (les plus vieilles d'abord) pour que les numéros se suivent logiquement
      const ordersToProcess = orders
        .filter(o => selectedOrders.includes(o.id))
        .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());

      // On récupère le compteur de départ
      let { year, sequence } = await getNextSequence();

      for (const order of ordersToProcess) {
        // Construction du numéro : FA2026-0000001
        const invoiceNum = `FA${year}-${String(sequence).padStart(7, '0')}`;
        
        // On incrémente pour le tour suivant
        sequence++;
            ...
            {activeTab === 'todo' && (
              <>
                <div className="flex flex-wrap gap-3 mb-6">
                  ...
                  <Button variant="outline" onClick={handleSelectCB} className="rounded-xl border-blue-200 text-blue-700 bg-blue-50 hover:bg-blue-100"><CreditCard className="w-4 h-4 mr-2" /> Cocher CB/Stripe</Button>
                ...
                {orders.length === 0 ? <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed"><CheckCircle className="h-12 w-12 text-green-500 mx-auto mb-3" /><p className="text-gray-600 font-medium">Tout est à jour !</p></div> : (
                  <div className="border rounded-xl overflow-hidden shadow-sm">
                    <table className="w-full text-sm text-left"><thead className="bg-gray-50 border-b text-gray-500 font-medium"><tr><th className="p-4 w-10"><Checkbox checked={selectedOrders.length === orders.length && orders.length > 0} onCheckedChange={handleSelectAll} /></th><th className="p-4">Date</th><th className="p-4">Client</th><th className="p-4">Montant</th><th className="p-4">Paiement</th></tr></thead>
                      <tbody className="divide-y">
                        {orders.map((order) => (
                          <tr key={order.id} className="hover:bg-[#FFF9F0] transition-colors cursor-pointer" onClick={() => setSelectedOrders(selectedOrders.includes(order.id) ? selectedOrders.filter(id => id !== order.id) : [...selectedOrders, order.id])}>
                            ...
                            <td className="p-4"><Badge variant="outline" className="bg-white">{order.payment_method || 'N/A'}</Badge></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
