...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ invoiceUrl, orderStatus: order.status, orderNumber: order.number }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
    return new Response(
      JSON.stringify({ error: error instanceof Error ? error.message : "Unknown error", invoiceUrl: null }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
});
