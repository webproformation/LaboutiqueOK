  ...
  try {
    ...
    switch (payload.table) {
      ...
      case 'customer_reviews':
        pathsToRevalidate.push('/');
        if (payload.record?.product_id) {
          pathsToRevalidate.push(`/product/*`);
        }
        break;
      ...
      case 'news_posts':
        ...
        if (payload.record?.slug) {
          pathsToRevalidate.push(`/actualites/${payload.record.slug}`);
        }
        break;
    ...
    // Appeler l'API de revalidation Next.js pour chaque path
