...
export default function DiagnosticSysteme() {
  ...
  const testRLSPolicies = async () => {
    ...
    try {
      ...
      const testCases = [
        ...
        { table: 'wishlist_items', operation: 'select', shouldWork: true },
      ];
  ...
  const testUserRoles = async () => {
    ...
    try {
      ...
      if (!session?.session) {
        addResult({
          name: 'Test des rôles utilisateur',
          status: 'warning',
          message: 'Non connecté - impossible de tester les rôles',
          time: Date.now() - startTime,
        });
        return;
      }
      ...
      addResult({
        name: 'Test des rôles utilisateur',
        status: 'success',
        message: roles && roles.length > 0 ? `Rôles trouvés: ${roles.map(r => r.role).join(', ')}` : 'Aucun rôle assigné',
        details: { userId: session.session.user.id, email: session.session.user.email, roles },
        time: Date.now() - startTime,
      });
    } catch (error: any) {
      addResult({
  ...
  const testWooCommerceAPI = async () => {
    const startTime = Date.now();
    try {
      const response = await fetch('/api/woocommerce/products?per_page=1');
      ...
      if (response.ok) {
        ...
        addResult({
          ...
          status: 'success',
          message: 'Connexion WooCommerce OK',
          details: { productsCount: data.length },
          time: Date.now() - startTime,
        });
      } else {
        addResult({
  ...
  const testWordPressAPI = async () => {
    const startTime = Date.now();
    try {
      const response = await fetch('/api/wordpress/posts?per_page=1');
      ...
      if (response.ok) {
        ...
        addResult({
          ...
          details: { postsCount: data.length },
          time: Date.now() - startTime,
        });
      } else {
        addResult({
    ...
    <div className="container mx-auto py-8 px-4 min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Diagnostic Complet du Système</h1>
            <p className="text-gray-600 mt-2">
              Test de tous les composants : Connexions, API, Base de données, Cache PostgREST, Rôles
            </p>
          </div>
          <Button
            onClick={runAllTests}
        ...
        <Card className="mt-6 bg-blue-50 border-blue-200">
          <CardContent className="p-6">
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              ...
              Actions recommandées si erreurs
            </h3>
            <ol className="list-decimal list-inside space-y-2 text-sm">
              <li>
                <strong>Erreurs 404 PostgREST :</strong>
                <br />
                <span className="text-gray-600 ml-5">
                  Dashboard Supabase → Settings → General → Pause project → Attendre 30s → Resume project
                </span>
              </li>
              <li>
                <strong>Problème de rôle admin :</strong>
                <br />
                <span className="text-gray-600 ml-5">
                  Utilisez la page /reset-password-admin pour réassigner le rôle admin à gregory.demeulenaere@gmail.com
                </span>
              </li>
              <li>
                ...
                <br />
                <span className="text-gray-600 ml-5">
                  Vérifiez votre fichier .env et assurez-vous que toutes les variables sont correctement définies
                </span>
              </li>
              <li>
                <strong>API WooCommerce/WordPress :</strong>
                <br />
                <span className="text-gray-600 ml-5">
                  Vérifiez les secrets dans Supabase Edge Functions et les variables WOOCOMMERCE_CONSUMER_KEY/SECRET
                </span>
              </li>
            </ol>
          </CardContent>
